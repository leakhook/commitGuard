...
test('warns on a secret-looking NEXT_PUBLIC_ value — NEXT_PUBLIC_ 에 시크릿처럼 보이는 값은 warn', () => {
...
test('no finding for a plain NEXT_PUBLIC_ value — NEXT_PUBLIC_ 에 평범한 값은 finding 없음', () => {
  assert.equal(run('NEXT_PUBLIC_SITE_NAME=My Blog').length, 0);
});

test('allowNextPublic=true disables the warning — allowNextPublic=true 면 경고 끔', () => {
  assert.equal(run('NEXT_PUBLIC_API_KEY=aB3xK9pQ7zR2mN5wT8uV1cD4', true).length, 0);
});

test('non-NEXT_PUBLIC_ variables are ignored by this rule — NEXT_PUBLIC_ 아닌 변수는 이 룰이 무시', () => {
